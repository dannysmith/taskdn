...
/// Build the Swift ↔ Rust bridge for Apple Intelligence.
///
/// Detects whether the current Xcode SDK includes the FoundationModels framework.
/// If it does, compiles the real implementation; otherwise compiles a stub that
/// returns "unavailable" for all calls.
///
/// The app uses weak linking for FoundationModels so it can launch on older macOS
/// versions — runtime availability is checked via @available(macOS 26.0, *).
#[cfg(all(target_os = "macos", target_arch = "aarch64"))]
