...
**Action:** The *principle* (inclusion/exclusion, OR for same filter, AND across filters) goes in S2. Specific flag syntax stays in CLI doc.
...
3. Split: S1 defines the *concept*, S2 defines *how to filter*
...
**Recommendation:** S2 recommends *concepts* (BM25 scoring, query syntax categories) without mandating specific libraries. Implementations choose their own tooling.
