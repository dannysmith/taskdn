    ...
    if (projects.length === 0) {
      ...
      for (const project of projects) {
        ...
        if (groupName === 'Done') {
          ...
          inProgressTasks.forEach((task: Task, i: number) => {
...
function getTaskParentChain(
  ...
  if (task.project) {
    ...
    project = result.projects.find((p: Project) => p.title.toLowerCase() === projectRef) ?? null;
  }

  // Find area (via project or direct)
  if (project && project.area) {
    const areaRef = project.area.replace(/^\[\[|\]\]$/g, '').toLowerCase();
    area = result.areas.find((a: Area) => a.title.toLowerCase() === areaRef) ?? null;
  } else if (task.area) {
  ...
  format(result: FormattableResult): string {
    switch (result.type) {
      ...
      case 'archived': {
        ...
        return archivedResult.dryRun ? `**Dry Run** - No changes made\n\n${output}` : output;
      }
      case 'batch-result': {
      ...
      case 'body-appended': {
        ...
        return appendedResult.dryRun ? `**Dry Run** - No changes made\n\n${output}` : output;
      }
      default:
        return `## ${result.type}\n\n(stub output)`;
    }
